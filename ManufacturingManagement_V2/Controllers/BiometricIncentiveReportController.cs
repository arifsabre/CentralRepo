...
{
    public class BiometricIncentiveReportController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Get_Biometric_Incentive(rptOptionMdl rptOption)
        {
            //[100146]
            if (objCookie.checkSessionState() == false) { return RedirectToAction("LoginUser", "Login"); };
            setViewData();
            ...
            {
                string reporturl = "BiometricIncentiveReport/get_Biometric_Incentive_File";
            ...
            return RedirectToAction("get_Biometric_Incentive_File", new { dtfrom = rptOption.DateFrom, dtto = rptOption.DateTo, compcode = rptOption.CompCode, grade = rptOption.Grade });
        }

        public ActionResult get_Biometric_Incentive_File(DateTime dtfrom, DateTime dtto, int compcode, string grade)
        {
            //[100146]
            ...
            rptDoc.Load(System.IO.Path.Combine(Server.MapPath("~/Reports"), "B_Incentive_Night_Shift.rpt"));//TestXCrystalReport1
            setLoginInfo(rptDoc);
            //dbp --ZZZ_USP_GET_FirstIn_Lastout_Night
