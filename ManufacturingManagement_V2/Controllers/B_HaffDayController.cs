...
{
    public class B_HaffDayController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Get_B_Hafday(rptOptionMdl rptOption)
        {
            if (objCookie.checkSessionState() == false) { return RedirectToAction("LoginUser", "Login"); };
            setViewData();
            ...
            {
                string reporturl = "B_Haffday/Get_B_Hafday_File";
            ...
            return RedirectToAction("Get_B_Hafday_File", new { dtfrom = rptOption.DateFrom, dtto = rptOption.DateTo, compcode = rptOption.CompCode, grade = rptOption.Grade });
        }
        public ActionResult Get_B_Hafday_File(DateTime dtfrom, DateTime dtto, int compcode, string grade)
        {
            //
            ...
            rptDoc.Load(System.IO.Path.Combine(Server.MapPath("~/Reports"), "B_Haffday_Report.rpt"));//TestXCrystalReport1
            setLoginInfo(rptDoc);
